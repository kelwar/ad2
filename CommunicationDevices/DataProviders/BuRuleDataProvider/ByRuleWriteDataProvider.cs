    ...
    public class ByRuleWriteDataProvider : IExchangeDataProvider<UniversalInputType, byte>
    {
        ...
        public byte[] GetDataByte()
        {
            try
            {
                ...
                if (requestFillBodyWithoutConstantCharacters.Contains("|") && requestFillBodyWithoutConstantCharacters.Contains("0x"))
                {
                    var str = requestFillBodyWithoutConstantCharacters.Split('|');
                    lenght = str.Length;
                ...
                if (resultStr.Contains("CheckSum"))
                    matchString = Regex.Match(resultStr, "(.*){CheckSum(.*)}").Groups[1].Value;
                else
                ...
                else
                {
                    ...
                    foreach (var s in str)
                    {
                        if (s.StartsWith("0x"))
                        {
                            ...
                            {
                                Log.log.Fatal($"Ошибка при попытке считать строку формата HEX: {s}. {ex.Message}");
                            }
                        }
                        else
                        {
                ...
                if (resultStr.Contains("CheckSum"))
                {
                    byte xor = CalcChecksumMod256(xorBytes);
                    resultStr = string.Format(resultStr.Replace("CheckSum", "0"), xor);
                }
                else if (resultStr.Contains("CRCXor"))
                {
                ...
                else
                {
                    ...
                    foreach (var s in str)
                    {
                        if (s.StartsWith("0x"))
                        {
                            ...
                            {
                                Log.log.Fatal($"Не удалось распознать ESC-последовательность в строке: {s}. {ex.Message}");
                            }
                        }
                        else
                        {
            ...
            {
                Log.log.Error($"Ошибка в нетабличном методе GetDataByte: {ex}");
                return null;
            }
        }
        ...
        private byte CalcChecksumMod256(IReadOnlyList<byte> arr)
        {
            var sum = 0;
